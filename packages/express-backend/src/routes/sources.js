...
sourcesRouter.post("/", async (req, res) => {
  try {
    ...
    if (source.active) {
      try {
        initialScrape = await scrapeOneSource(source);
      } catch (scrapeErr) {
        console.error(`[sources] initial scrape failed for ${source.url}`, scrapeErr.message);
        initialScrape = { error: "Initial scrape failed; source was still saved." };
      }
    }

    res.status(201).json({ ...source, initialScrape });
  } catch (err) {
    if (err.code === 11000) {
